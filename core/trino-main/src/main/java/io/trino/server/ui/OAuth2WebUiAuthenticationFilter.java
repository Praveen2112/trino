        ...
        implements WebUiAuthenticationFilter
{
    ...
    @Inject
    public OAuth2WebUiAuthenticationFilter(OAuth2Service service, OAuth2Client client, TokenPairSerializer tokenPairSerializer, @ForRefreshTokens Duration refreshWindow, OAuth2Config oauth2Config)
    {
        ...
        if (refreshToken.isPresent()) {
            try {
                ...
                request.abortWith(Response.seeOther(request.getUriInfo().getRequestUri())
                        .cookie(OAuthWebUiCookie.create(serializedToken, response.getExpiration().plus(refreshWindow)))
                        .build());
            }
