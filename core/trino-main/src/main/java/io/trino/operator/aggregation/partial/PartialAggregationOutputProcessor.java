...
public class PartialAggregationOutputProcessor
{
    ...
    private final int[] optionalState;
    ...
    public PartialAggregationOutputProcessor(
            ...
            Optional<List<Integer>> intermediateState)
    {
        ...
        if (intermediateState.isPresent()) {
            optionalState = Ints.toArray(intermediateState.get());
        }
        else {
            optionalState = new int[0];
        }
    }
    ...
    public Page processRawInputPage(Page page)
    {
        ...
        if (step == AggregationNode.Step.INTERMEDIATE) {
            for (int i = 0; i < optionalState.length; i++, blockOffset++) {
                finalPage[blockOffset] = page.getBlock(optionalState[i]);
            }
        }
        else {
